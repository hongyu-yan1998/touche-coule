...
const ships = [main.myShipDestroyer(), main.myShipBattleship(), main.myShipDestroyer1(), main.myShipBattleship1()]
var indexShip = 0
const Buttons = ({ wallet }: { wallet: ReturnType<typeof useWallet> }) => {
  const next = () => wallet?.contract.turn()
  const nextResgister = () => {
    const address = ships[indexShip % ships.length]
    indexShip++
    wallet?.contract.register(address)
  }
  return (
    <div style={{ display: 'flex', gap: 5, padding: 5 }}>
      <button onClick={nextResgister}>Register</button>
